    ...
    create(request, response) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const res = yield connect_1.prisma.user.findUnique({ where: { email } }); //faz busca no banco com o email
                if (!res == null) {
                ...
                const user = yield connect_1.prisma.user.create({
                    //cria user
                    data: {
                        name,
                        email,
                ...
                yield connect_1.prisma.valor.create({
                ...
                const data = jsonwebtoken_1.default.sign({ id: user.id, name: name }, "190526", {
    ...
    valid(request, response) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const user = yield connect_1.prisma.user.findUnique({ where: { email } });
                if (!user) {
                    return response.status(401).json({ erro: "Usuário não encontrado" });
                }
                const hash = yield connect_1.prisma.user.findUnique({
                    where: { email: email },
    ...
    googlevalid(request, response) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const user = yield connect_1.prisma.user.findUnique({ where: { email } }); //busca no banco
                if (!user) {
                    return response.status(401).json({ erro: "Usuário não encontrado" });
                }
                const data = jsonwebtoken_1.default.sign({ id: user.id, name: user.name }, "190526", {
    ...
    all(request, response) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const data = yield connect_1.prisma.user.findMany({
                    where: {
                        NOT: {
